...
| フラグ | 動作 |
|---|---|
| (なし) | **全ての未割当変更**をコミット（staged + unstaged）。注意が必要 |
| `--only` | **staged済みファイルのみ**コミット。`but stage`で事前にステージング必須 |
| `--changes <id1,id2,...>` | **指定IDのファイルのみ**コミット。`--only`と排他 |
| `--files <id>` | 特定ファイルIDのみ |

**重要**: `but commit`（フラグなし）は`git commit`と異なり、ステージング状態に関わらず全変更を含む。
クリーンなコミットを作るには `--only` か `--changes` を使うこと。
