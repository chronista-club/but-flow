...
| コマンド | 説明 |
|---|---|
| `but commit -m "msg"` | コミット（**全未割当変更**。注意: staged/unstaged問わず全て含む） |
| `but commit --only -m "msg"` | **staged済みファイルのみ**コミット（`but stage`で事前ステージング必須） |
| `but commit --changes <id1,id2> -m "msg"` | **指定IDのファイルのみ**コミット |
