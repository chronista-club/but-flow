...
def main():
    ...
    print("\n-- git branch: 読み取り専用は許可 --")
    results.append(test("git branch (一覧)", "git branch", False))
    results.append(test("git branch --show-current", "git branch --show-current", False))
    results.append(test("git branch --list", "git branch --list", False))
    results.append(test("git branch -l", "git branch -l", False))
    results.append(test("git branch -v", "git branch -v", False))
    results.append(test("git branch --verbose", "git branch --verbose", False))
    results.append(test("git branch -a", "git branch -a", False))
    results.append(test("git branch --all", "git branch --all", False))
    results.append(test("git branch -r", "git branch -r", False))
    results.append(test("git branch --remotes", "git branch --remotes", False))
    results.append(test("git branch --contains abc", "git branch --contains abc123", False))
    results.append(test("git branch --merged", "git branch --merged", False))
    results.append(test("git branch --no-merged", "git branch --no-merged", False))

    print("\n-- git branch: 書き込み操作はブロック --")
    results.append(test("git branch -d feature", "git branch -d feature", True))
    results.append(test("git branch -D feature", "git branch -D feature", True))
    results.append(test("git branch --delete feature", "git branch --delete feature", True))
    results.append(test("git branch -m old new", "git branch -m old new", True))
    results.append(test("git branch -M old new", "git branch -M old new", True))
    results.append(test("git branch --move old new", "git branch --move old new", True))
    results.append(test("git branch -c old new", "git branch -c old new", True))
    results.append(test("git branch --copy old new", "git branch --copy old new", True))
    results.append(test("git branch --force x", "git branch --force feature", True))
    results.append(test("git branch new-branch (作成)", "git branch new-branch", True))
